...
DROP POLICY IF EXISTS "کاربران می‌توانند پروفایل خود را ویرایش کنند" ON users;

CREATE POLICY "کاربران می‌توانند پروفایل خود را ویرایش کنند"
  ON users FOR UPDATE
  TO authenticated
  USING (auth.uid() = id)
  WITH CHECK (
    auth.uid() = id AND
    (SELECT username FROM users WHERE id = auth.uid()) = username AND
    (SELECT email FROM users WHERE id = auth.uid()) = email AND
    (SELECT role FROM users WHERE id = auth.uid()) = role AND
    (SELECT is_active FROM users WHERE id = auth.uid()) = is_active
  );
